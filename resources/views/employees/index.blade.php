...
@section('content')
    ...
    <header>
    <nav>
        <h2 style="margin: 0;">Welcome!</h2> <!-- Adjusted to remove inline styles -->
...
<div class="d-flex justify-content-between align-items-center" id="searchBarContainer">
    <div class="col-md-8">
        <input type="text" id="search" class="form-control mb-3" placeholder="Search by name or email..." onkeyup="searchTable()">
    </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="{{ route('employees.create') }}" class="btn btn-primary btn-xs float-end py-0">Create User</a>
                        <table class="table table-responsive table-bordered table-striped" id="employeeTable" style="margin-top:10px;">
                            <thead>
                                <tr>
                                    <th>S.NO</th>
                                    <th>Name</th>
                            ...
                            <tbody id="employeeTableBody">
                                @foreach($employees as $key => $employee)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="btn {{ $employee->is_active == 1 ? 'btn-success' : 'btn-danger' }} btn-xs py-0">
                                                {{ $employee->is_active == 1 ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                        <td>
                                            <div class="d-inline">
                                                <a href="{{ route('employees.show', $employee->id) }}" class="btn btn-primary btn-sm mx-2">Show</a>
                                                <a href="{{ route('employees.edit', $employee->id) }}" class="btn btn-warning btn-sm mx-2">Edit</a>
                                                <form action="{{ route('employees.destroy', $employee->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this User?');" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <!-- JavaScript for Filtering and Dropdown Menu -->
    <script>
    ...
    <!-- CSS for Sidebar, Navbar, and Dropdown -->
    <style>
        .sidenav {
        ...
        .sidenav a:hover {
            background-color:blue; /* Highlight on hover */
        }
        nav {
...
.profile-icon {
    ...
    margin-left: 20px; /* Space between the text and icon */
}
